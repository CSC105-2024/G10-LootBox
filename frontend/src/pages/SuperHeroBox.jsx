...
const Main = () => {
  ...
  const [heroKeys, setHeroKeys] = useState(0);
  const [nextHeroKeyTime, setNextHeroKeyTime] = useState(null);
  ...
  const fetchData = async () => {
    ...
    if (res.success && res.data) {
      ...
      const heroKey = keys.find(key => key.keyType === "Superhero");
      setHeroKeys(heroKey?.count || 0);
      setNextHeroKeyTime(heroKey?.nextRegenerateTime || null);
    } else {
  ...
  useEffect(() => {
    if (!nextHeroKeyTime) return;
  
    const interval = setInterval(() => {
      const now = new Date();
      const target = new Date(nextHeroKeyTime);
      ...
      {!isMobile ? (
        <div className="flex flex-1">
          ...
          <div className="w-1/4 text-left px-4 pt-16">
            ...
            <p className="font-pixelify text-[#0CC2FF] text-4xl mb-2" style={textStrokeStyle}>rare 38.9%</p>
            <p className="font-pixelify text-[#9C4BFF] text-4xl mb-2" style={textStrokeStyle}>epic 20%</p>
            <p className="font-pixelify text-[#EAB70F] text-4xl mb-2" style={textStrokeStyle}>legendary 1%</p>
          ...
          <div className="w-1/3 flex flex-col items-center justify-center">
            ...
            <div className="flex items-center justify-center mb-4">
              ...
              <span 
                className={`font-pixelify text-lg ${heroKeys > 0 ? 'text-[#00DA62]' : 'text-red-600'}`} 
                style={textStrokeStyle}
              >
                Key remaining {heroKeys}
              </span>
            </div>
        ...
        <div className="flex flex-col items-center flex-1">
          ...
          <div className="flex items-center justify-center mb-2">
            ...
            <span 
              className={`font-pixelify text-sm ${heroKeys > 0 ? 'text-[#00DA62]' : 'text-red-600'}`} 
              style={textStrokeStyle}
            >
              Key remaining {heroKeys}
            </span>
